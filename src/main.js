...
window.onload= fetcheame
 ...
 function fetcheame () {
       ...
       let gender= document.getElementById("select-gender-category").value;
        fetch ("https://api.themoviedb.org/3/discover/movie?api_key=48819a4f88e3d597df63bebab6723d0f&with_genres="+gender)
          .then (data=>data.json())
          .then (data =>{
            allData = data.results;
           console.log("https://api.themoviedb.org/3/discover/movie?api_key=48819a4f88e3d597df63bebab6723d0f&with_genres="+gender)
           for (let i = 0; i <allData.length; i++){
           document.getElementById('content1').innerHTML += 
           `
  <div class="col s6 m3">
    <div id="card1" class="card medium">
      ...
      <div class="card-content">
        <p class="title-name card-title ">${allData[i].title}</p>
        <p> Año: ${allData[i].release_date}</p>
      </div>
    </div>
  </div> 
`
